...
/usr/sbin/parted /dev/mmcblk0 mklabel gpt << EOF
y
EOF

#Create boot files partition
/usr/sbin/parted /dev/mmcblk0 mkpart primary fat32 ${BOOT_PARTITION_START}MB ${PERSISTENT_PARTITION_START}MB
#Create persistent data partition
/usr/sbin/parted /dev/mmcblk0 mkpart primary ext2 ${PERSISTENT_PARTITION_START}MB ${ROOTFS1_PARTITION_START}MB
#Create first rootfs partition
/usr/sbin/parted /dev/mmcblk0 mkpart primary ext4 ${ROOTFS1_PARTITION_START}MB ${ROOTFS2_PARTITION_START}MB
#Create second rootfs partition
/usr/sbin/parted /dev/mmcblk0 mkpart primary ext4 ${ROOTFS2_PARTITION_START}MB ${APPS_PARTITION_START}MB
#Create apps partition
/usr/sbin/parted /dev/mmcblk0 mkpart primary ext4 ${APPS_PARTITION_START}MB ${DATA_PARTITION_START}MB
#Create data partition
/usr/sbin/parted /dev/mmcblk0 mkpart primary ext4 ${DATA_PARTITION_START}MB 100%
#Print partition layout
/usr/sbin/parted /dev/mmcblk0 print
